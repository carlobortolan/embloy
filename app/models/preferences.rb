class Preferences < ApplicationRecord
  ...
  validates :gender, inclusion: { in: %w[male female other], "error": "ERR_INVALID", "description": "Attribute is invalid" }, presence: false
  validates :spontaneity, presence: false, :numericality => { greater_than_or_equal_to: 0, less_than_or_equal_to: 100, "error": "ERR_INVALID", "description": "Attribute is malformed or unknown" }
  ...
  def salary_range_validation
    return unless !salary_range.nil? && salary_range.present? && salary_range.size == 2 && !salary_range[1].nil? && !salary_range[0].nil?
    unless salary_range[1] >= salary_range[0]
      errors.add(:salary_range, { "error": "ERR_INVALID", "description": "Lower bound must be smaller or equal than upper bound" })
    end
  end
end