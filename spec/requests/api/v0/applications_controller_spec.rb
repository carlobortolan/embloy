...
RSpec.describe 'ApplicationsController' do
  before(:all) do
    ...
    allowed_cv_formats = [%w[pdf docx txt xml], ['pdf'], ['docx'], ['txt'], ['xml'], ['pdf'], ['docx'], ['txt'], ['xml']]
    ...
    12.times do |i|
      ...
      @jobs[i].application_options.create!(
        question: 'TestOption',
        question_type: 'file',
        required: cv_required[i],
        options: allowed_cv_formats[i]
      )
    end
    6.times do |i|
  ...
  describe 'Applications', type: :request do
    ...
    describe '(POST: /api/v0/jobs/{id}/applications)' do
      ...
      let(:valid_attributes_docx) do
        {
          application_answers: {
            '0' => {
              application_option_id: 1,
              file: Rack::Test::UploadedFile.new(Rails.root.join('spec/assets', 'test_file.docx'), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
            }
          }
        }
      end
      let(:valid_attributes_txt) do
        {
          application_answers: {
            '0' => {
              application_option_id: 1,
              file: Rack::Test::UploadedFile.new(Rails.root.join('spec/assets', 'test_file.txt'), 'text/plain')
            }
          }
        }
      end
      ...
      context 'valid inputs' do
        it 'returns [201 Created] for successful application not requiring cv' do
          post("/api/v0/jobs/#{@jobs[0].id}/applications", params: valid_attributes_basic, headers:)
          expect(response).to have_http_status(201)
        end
        it 'returns [201 Created] for successful application requiring cv with \'pdf\' format' do
          post("/api/v0/jobs/#{@jobs[1].id}/applications",
               params: { application_answers: { '0' => valid_attributes_pdf[:application_answers]['0'].merge(application_option_id: @jobs[1].application_options.first.id) } }, headers:)
          expect(response).to have_http_status(201)
        end
        it 'returns [201 Created] for successful application requiring cv with \'docx\' format' do
          post("/api/v0/jobs/#{@jobs[2].id}/applications",
               params: { application_answers: { '0' => valid_attributes_docx[:application_answers]['0'].merge(application_option_id: @jobs[2].application_options.first.id) } }, headers:)
          expect(response).to have_http_status(201)
        end
        it 'returns [201 Created] for successful application requiring cv with \'txt\' format' do
          post("/api/v0/jobs/#{@jobs[3].id}/applications",
               params: { application_answers: { '0' => valid_attributes_txt[:application_answers]['0'].merge(application_option_id: @jobs[3].application_options.first.id) } }, headers:)
          expect(response).to have_http_status(201)
        end
        it 'returns [201 Created] for successful application requiring cv with \'xml\' format' do
          post("/api/v0/jobs/#{@jobs[4].id}/applications",
               params: { application_answers: { '0' => valid_attributes_xml[:application_answers]['0'].merge(application_option_id: @jobs[4].application_options.first.id) } }, headers:)
          expect(response).to have_http_status(201)
        end
      end
      ...
      context 'invalid inputs' do
        ...
        it 'returns [401 Unauthorized] for expired/invalid access token' do
          ...
          post("/api/v0/jobs/#{@jobs[0].id}/applications", params:, headers:)
          expect(response).to have_http_status(401)
        end
        it 'returns [403 Forbidden] for blacklisted user' do
          headers = { 'Authorization' => "Bearer #{@valid_at_blacklisted}" }
        ...
        it 'returns [404 Not Found] if job does not exist' do
          headers = { 'Authorization' => "Bearer #{@valid_access_token}" }
          params = { application_answers: { '0' => valid_attributes_docx[:application_answers]['0'].merge(application_option_id: 123_123_123) } }
          post('/api/v0/jobs/123123123/applications', params:, headers:)
          expect(response).to have_http_status(404)
        end
