...
ActiveRecord::Schema[7.0].define(version: 2024_11_14_005852) do
  ...
  create_enum "job_status", ["listed", "unlisted", "archived"]
  create_enum "notify_type", ["0", "1"]
  create_enum "question_type", ["yes_no", "short_text", "long_text", "number", "link", "single_choice", "multiple_choice", "location", "file", "date"]
  ...
  create_table "job_list_items", force: :cascade do |t|
    ...
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["job_id", "job_list_id"], name: "index_job_list_items_on_job_id_and_job_list_id", unique: true
  ...
  create_table "jobs", primary_key: "job_id", id: :serial, force: :cascade do |t|
    ...
    t.index "to_tsvector('simple'::regconfig, (((((((((((((((((COALESCE(title, ''::character varying))::text || ' '::text) || (COALESCE(job_type, ''::character varying))::text) || ' '::text) || (COALESCE(\"position\", ''::character varying))::text) || ' '::text) || (COALESCE(key_skills, ''::character varying))::text) || ' '::text) || COALESCE(description, ''::text)) || ' '::text) || (COALESCE(country_code, ''::character varying))::text) || ' '::text) || (COALESCE(city, ''::character varying))::text) || ' '::text) || (COALESCE(postal_code, ''::character varying))::text) || ' '::text) || (COALESCE(address, ''::character varying))::text))", name: "jobs_tsvector_idx", using: :gin
